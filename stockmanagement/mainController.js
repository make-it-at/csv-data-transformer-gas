...
var MainController = {
  ...
  updateReferenceInfoBatched: function(startFromIndex = 0) {
    try {
      ...
      // シートとデータの取得
      const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('_Reference');
      if (!sheet) {
        throw new Error('_Referenceシートが見つかりません');
      }
      ...
      // 設定を読み込み
      const config = ConfigManager.getUpdateConfig();
      LoggerManager.info('設定を読み込みました', config);
      
      // 有効なデータ行を抽出（ヘッダー行を除く）
      const validData = referenceData.slice(1).filter(row => row[0] && row[0].toString().trim() !== '');
      
      if (validData.length === 0) {
        LoggerManager.info('更新対象のデータがありません');
        return 'NoData';
      }
      
      // 処理対象の銘柄コードを準備
      const stockItems = validData.map((row, index) => ({
        code: row[columnIndices['コード'] - 1],
        rowIndex: index + 2, // ヘッダー行 + 1ベースのインデックス
        data: row
      }));
      
      LoggerManager.info(`更新対象の銘柄数: ${stockItems.length}件`);
      ...
      const batchOptions = {
        ...
        processId: 'reference_update_batch'  // 固定のprocessIdで処理状態を管理
      };
      
      // 個別銘柄処理関数
      const stockProcessor = (stockItem, index, allItems) => {
        try {
          const code = stockItem.code;
          const rowIndex = stockItem.rowIndex;
          
          LoggerManager.debug(`${code}の情報を更新中...`);
          
          // 各項目の情報を取得
          const updates = {};
          
          // 配当金情報
          if (config.dividend.enabled) {
            try {
              const dividendInfo = this.fetchDividendInfo(code, config.dividend);
              if (dividendInfo) {
                updates[columnIndices['配当金'] - 1] = dividendInfo;
              }
            } catch (error) {
              LoggerManager.warn(`${code}の配当金情報取得失敗:`, error.message);
            }
          }
          
          // 分配金情報（投資信託のみ）
          if (config.distribution.enabled && this.isInvestmentTrust(code)) {
            try {
              const distributionInfo = this.fetchDistributionInfo(code, config.distribution);
              if (distributionInfo) {
                updates[columnIndices['分配金'] - 1] = distributionInfo;
              }
            } catch (error) {
              LoggerManager.warn(`${code}の分配金情報取得失敗:`, error.message);
            }
          }
          ...
          if (config.eps.enabled) {
            try {
              const epsInfo = this.fetchEPSInfo(code, config.eps);
              if (epsInfo) {
                updates[columnIndices['EPS(予想)'] - 1] = epsInfo;
              }
            } catch (error) {
              LoggerManager.warn(`${code}のEPS情報取得失敗:`, error.message);
            }
          }
          
          // BPS情報
          if (config.bps.enabled) {
            try {
              const bpsInfo = this.fetchBPSInfo(code, config.bps);
              if (bpsInfo) {
                updates[columnIndices['BPS'] - 1] = bpsInfo;
              }
            } catch (error) {
              LoggerManager.warn(`${code}のBPS情報取得失敗:`, error.message);
            }
          }
          ...
          if (config.rights.enabled) {
            try {
              const rightsInfo = this.fetchRightsInfo(code, config.rights);
              if (rightsInfo) {
                updates[columnIndices['権利確定'] - 1] = rightsInfo;
              }
            } catch (error) {
              LoggerManager.warn(`${code}の権利確定情報取得失敗:`, error.message);
            }
          }
          ...
          if (Object.keys(updates).length > 0) {
            Object.keys(updates).forEach(colIndex => {
              referenceData[rowIndex - 1][colIndex] = updates[colIndex];
            });
            
            // シートに即座に反映
            const updateRange = sheet.getRange(rowIndex, 1, 1, referenceData[rowIndex - 1].length);
            updateRange.setValues([referenceData[rowIndex - 1]]);
            
            LoggerManager.info(`${code}の情報を更新しました: ${Object.keys(updates).length}項目`);
            return { success: true, updatedItems: Object.keys(updates).length };
          }
          
          return { success: true, updatedItems: 0 };
          
        } catch (error) {
          LoggerManager.error(`${stockItem.code}の処理中にエラー:`, error);
          return { success: false, error: error.message };
        }
      };
      
      // バッチ処理を実行
      const result = BatchProcessor.processInBatches(stockItems, stockProcessor, batchOptions);
      
      // 結果の処理
      if (result.status === 'completed') {
        LoggerManager.info(`_Reference情報更新が完了しました: ${result.processedCount}件処理, ${result.successCount}件成功, ${result.errorCount}件エラー`);
        UIManager.updateProgress(result.processedCount, stockItems.length, '更新が完了しました', {
          phase: '完了',
        ...
        LoggerManager.warn(`_Reference情報更新が中断されました: ${result.status}`, result);
        UIManager.updateProgress(result.processedCount, stockItems.length, 
          `処理が中断されました（${result.processedCount}/${stockItems.length}件完了）`, {
            phase: '中断',
   ...
   * 列インデックスを取得
   */
  getColumnIndices: function(headerRow) {
    const indices = {};
    const requiredColumns = ['コード', '配当金', '分配金', 'EPS(予想)', 'BPS', '権利確定'];
    ...
    requiredColumns.forEach(column => {
      ...
      indices[column] = index + 1; // 1ベースのインデックス
    });
   ...
   * 投資信託かどうかを判定
   */
  isInvestmentTrust: function(code) {
    // 投資信託のコードパターン（例: 4桁の数字 + アルファベット）
   ...
   * 配当金情報を取得
   */
  fetchDividendInfo: function(code, config) {
    try {
      const url = config.url.replace('{code}', code);
      const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
      const html = response.getContentText();
      
      LoggerManager.debug(`${code}の配当金情報を取得中... URL: ${url}`);
      LoggerManager.debug(`${code}の配当金情報取得: HTML先頭100文字: ${html.substring(0, 100)}`);
      
      const match = html.match(new RegExp(config.regex));
      if (match) {
        const dividend = parseInt(match[1]) + (match[2] ? parseInt(match[2]) / 10 : 0);
        LoggerManager.info(`${code}の配当金情報を取得しました: ${dividend}`);
        return dividend;
      } else {
        LoggerManager.debug(`${code}の配当金情報が取得できませんでした。正規表現: ${config.regex}`);
        LoggerManager.debug(`${code}の配当金情報取得失敗: HTML抜粋: ${html.substring(0, 500)}`);
        return null;
      }
    } catch (error) {
      LoggerManager.error(`${code}の配当金情報取得エラー:`, error);
      return null;
    }
  },
   ...
   * 分配金情報を取得
   */
  fetchDistributionInfo: function(code, config) {
    try {
      console.log(`分配金取得開始: ${code}`);
      console.log(`設定URL: ${config.url}`);
      console.log(`設定正規表現: ${config.regex}`);
      
      const url = config.url.replace('{code}', code);
      console.log(`実際のURL: ${url}`);
      
      const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
      const html = response.getContentText();
      
      console.log(`HTTPレスポンス: ${response.getResponseCode()}`);
      console.log(`HTML長さ: ${html.length}`);
      
      LoggerManager.debug(`${code}の分配金情報を取得中... URL: ${url}`);
      LoggerManager.debug(`${code}の分配金情報取得: HTML先頭100文字: ${html.substring(0, 100)}`);
      
      // 複数の正規表現パターンを試す（現在のYahoo Financeに対応）
      const distributionPatterns = [
        /予想分配金\s*[:：]\s*([\d,\.]+)円/, // 予想分配金ラベル
        /予想分配金<\/dt>[\s\S]*?<dd[^>]*>([\d,\.]+)円<\/dd>/, // dt/dd形式
        /予想分配金<\/th>[\s\S]*?<td[^>]*>([\d,\.]+)円<\/td>/, // table形式
        /"dividend":"([\d,\.]+)"/, // JSON形式
        /<span class="_3RQJDhPQ">予想分配金<\/span>[\s\S]*?<span class="_3rXWJKZF _11kV6f2G">([\d,\.]+)<\/span>/, // 既存パターン（後方互換）
        /分配金\s*[:：]\s*([\d,\.]+)円/, // 一般的な分配金ラベル
        /class="[^"]*dividend[^"]*"[^>]*>([\d,\.]+)/, // dividendクラス
        /<span[^>]*>\s*([\d,\.]+)\s*円<\/span>/, // 汎用spanパターン
        /<span[^>]*DataListItem__name[^>]*>予想分配金<\/span>[\s\S]*?<span[^>]*StyledNumber__value[^>]*>([\d,\.]+)<\/span>/ // 最新Yahoo Finance対応
      ];
      ...
      // まず設定されたパターンを試す
      if (config.regex) {
        console.log(`設定パターンを試行: ${config.regex}`);
        const match = html.match(new RegExp(config.regex));
        if (match && match[1]) {
          const cleanValue = match[1].replace(/,/g, '');
          if (!isNaN(cleanValue) && cleanValue !== '') {
            distribution = parseFloat(cleanValue);
            console.log(`設定パターンで取得成功: ${distribution}`);
            LoggerManager.info(`${code}の分配金情報を設定パターンで取得: ${distribution}`);
          }
        } else {
          console.log(`設定パターンマッチ失敗`);
        }
      }
      
      // 設定パターンで取得できない場合は代替パターンを試す
      if (!distribution) {
        console.log(`代替パターンを試行中...`);
        for (let i = 0; i < distributionPatterns.length; i++) {
          console.log(`パターン ${i}: ${distributionPatterns[i]}`);
          const match = html.match(distributionPatterns[i]);
          if (match && match[1]) {
            console.log(`パターン ${i} マッチ: ${match[1]}`);
            const cleanValue = match[1].replace(/,/g, '');
            if (!isNaN(cleanValue) && cleanValue !== '') {
              distribution = parseFloat(cleanValue);
              console.log(`代替パターン ${i} で取得成功: ${distribution}`);
              LoggerManager.info(`${code}の分配金情報を取得しました`, {
                value: distribution,
                pattern: i,
      ...
      if (!distribution) {
        const errorMsg = `${code}の分配金情報が取得できませんでした`;
        console.log(errorMsg);
        console.log(`試行パターン数: ${distributionPatterns.length}`);
        console.log(`HTML先頭500文字: ${html.substring(0, 500)}`);
        LoggerManager.warn(errorMsg, {
          patterns: distributionPatterns.length,
      ...
      LoggerManager.error(`${code}の分配金情報取得エラー:`, error);
      return null;
    }
  },
   ...
   * EPS情報を取得
   */
  fetchEPSInfo: function(code, config) {
    try {
      const url = config.url.replace('{code}', code);
      const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
      const html = response.getContentText();
      
      LoggerManager.debug(`${code}のEPS情報を取得中... URL: ${url}`);
      LoggerManager.debug(`${code}のEPS情報取得: HTML先頭100文字: ${html.substring(0, 100)}`);
      
      const match = html.match(new RegExp(config.regex));
      if (match) {
        const eps = parseFloat(match[1].replace(/,/g, ''));
        LoggerManager.info(`${code}のEPS情報を取得しました: ${eps}`);
        return eps;
      } else {
        LoggerManager.debug(`${code}のEPS情報が取得できませんでした。正規表現: ${config.regex}`);
        LoggerManager.debug(`${code}のEPS情報取得失敗: HTML抜粋: ${html.substring(0, 500)}`);
        return null;
      }
    } catch (error) {
      LoggerManager.error(`${code}のEPS情報取得エラー:`, error);
      return null;
    }
  },
   ...
   * BPS情報を取得
   */
  fetchBPSInfo: function(code, config) {
    try {
      const url = config.url.replace('{code}', code);
      const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
      const html = response.getContentText();
      
      LoggerManager.debug(`${code}のBPS情報を取得中... URL: ${url}`);
      LoggerManager.debug(`${code}のBPS情報取得: HTML先頭100文字: ${html.substring(0, 100)}`);
      
      const match = html.match(new RegExp(config.regex));
      if (match) {
        const bps = parseInt(match[1].replace(/,/g, ''));
        LoggerManager.info(`${code}のBPS情報を取得しました: ${bps}`);
        return bps;
      } else {
        LoggerManager.debug(`${code}のBPS情報が取得できませんでした。正規表現: ${config.regex}`);
        LoggerManager.debug(`${code}のBPS情報取得失敗: HTML抜粋: ${html.substring(0, 500)}`);
        return null;
      }
    } catch (error) {
      LoggerManager.error(`${code}のBPS情報取得エラー:`, error);
      return null;
    }
  },
   ...
   * 権利確定情報を取得
   */
  fetchRightsInfo: function(code, config) {
    try {
      const url = config.url.replace('{code}', code);
      const response = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
      const html = response.getContentText();
      
      LoggerManager.debug(`${code}の権利確定情報を取得中... URL: ${url}`);
      LoggerManager.debug(`${code}の権利確定情報取得: HTML先頭100文字: ${html.substring(0, 100)}`);
      
      const match = html.match(new RegExp(config.regex));
      if (match) {
        const rights = match[1].trim();
        LoggerManager.info(`${code}の権利確定情報を取得しました: ${rights}`);
        return rights;
      } else {
        LoggerManager.debug(`${code}の権利確定情報が取得できませんでした。正規表現: ${config.regex}`);
        LoggerManager.debug(`${code}の権利確定情報取得失敗: HTML抜粋: ${html.substring(0, 500)}`);
        return null;
      }
    } catch (error) {
      LoggerManager.error(`${code}の権利確定情報取得エラー:`, error);
      return null;
    }
  },
};
